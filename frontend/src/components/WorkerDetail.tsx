...
export function WorkerDetail({worker, devices, onRefresh}: Props) {
  ...
  const sendAlert = async () => {
    await api.sendAlert(av?.device_id);
    await onRefresh();
  };
  return (
    <aside className="worker-detail">
      <div className="detail-head">
        <div>
          ...
          <p>{worker.worker_id} · {worker.helmet_id}</p>
        </div>
        <div className={`risk-orb risk-${worker.risk_level}`}>
          <strong>{worker.risk_score}</strong>
          <span>{worker.risk_level}</span>
        </div>
      </div>
      <div className="detail-grid">
        ...
        <div><span>보호구</span><strong>조끼 {worker.ppe.vest ? "착용" : "미착용"} · 장갑 {worker.ppe.glove ? "착용" : "미착용"}</strong></div>
      </div>
      <section>
        <div className="section-title"><h3>위험도 근거</h3><span>{worker.risk_reasons.length}개</span></div>
        <div className="reason-list">
          {worker.risk_reasons.length ? worker.risk_reasons.map(item => (
            <div key={item.reason}><span>{item.reason}</span><b>+{item.points}</b></div>
          )) : <p className="empty">감지된 위험 요인이 없습니다.</p>}
        </div>
      </section>
      <section>
      ...
      <button className="alert-button" onClick={() => void sendAlert()}>
        <span>!</span> 안전모에 경고 보내기
      </button>
    </aside>
  );
}
