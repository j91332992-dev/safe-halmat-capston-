...
현재 담당 범위는 Makerfabs ESP32 UWB V1.1/DW3000 거리측정, Python 좌표 계산, 5.80m × 8.20m 위치지도입니다.

첫 완료 목표는 보드 2개로 1m·2m·3m 거리값을 시리얼에서 반복 측정하는 것입니다. 다음으로 Anchor 4개를 공간 모서리에 배치하고, 마지막에 Python 서버와 지도를 연결합니다. 보드 펌웨어만 검증된 DW3000 C/C++ 드라이버를 사용하며 Arduino IDE는 사용하지 않습니다.

Anchor 설치 좌표: `A1=(0,0)`, `A2=(5.80,0)`, `A3=(5.80,8.20)`, `A4=(0,8.20)`.
...
2. `configure_uwb_test_site.py`를 실행해 5.80m × 8.20m 지도와 Anchor 좌표를 적용합니다.
